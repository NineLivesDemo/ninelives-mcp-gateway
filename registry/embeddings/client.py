...
def _extract_openai_embeddings(
    ...
    """Extract vectors from a standard OpenAI embeddings response envelope."""
    if not isinstance(payload, dict) or not isinstance(payload.get("data"), list):
        raise ValueError("OpenAI embeddings response did not contain a 'data' list")

    vectors: list[list[float]] = []
    for item in payload["data"]:
        if not isinstance(item, dict) or not isinstance(item.get("embedding"), list):
            raise ValueError("OpenAI embeddings response contained a malformed data item")
        vectors.append(item["embedding"])

    if not vectors:
        raise ValueError("OpenAI embeddings response contained no vectors")
    return vectors
...
class LiteLLMClient(EmbeddingsClient):
    ...
    def _encode_direct(
        ...
        *,
        ...
        """Call an OpenAI-compatible endpoint directly and extract its vectors.

        Direct mode avoids provider-specific request rewriting in LiteLLM. The
        token (IdP bearer or static api_key) is injected exactly as in the
        LiteLLM path. api_base is operator-configured trusted config, not user
        input.
        """
        import httpx
        ...
        if not self.api_base:
            raise RuntimeError(
                f"EMBEDDINGS_RESPONSE_FORMAT={response_format} requires EMBEDDINGS_API_BASE "
                "(the OpenAI-compatible endpoint to call directly)."
            )
        url = self.api_base.rstrip("/") + "/embeddings"
        # Direct proxy calls must preserve the configured model alias. The legacy
        # raw_array path retains its provider-prefix stripping behavior.
        model = (
            self.model_name
            if response_format == "openai_http"
            else self.model_name.split("/", 1)[-1]
        )
        ...
        try:
            logger.debug("Calling direct embeddings endpoint: %s", url)
            response = httpx.post(
                url,
            ...
            vectors = (
                _extract_openai_embeddings(payload)
                if response_format == "openai_http"
                else _extract_raw_array(payload)
            )
            embeddings_array = np.array(vectors, dtype=np.float32)
        except httpx.HTTPStatusError as e:
            logger.error(
                "Direct embeddings endpoint returned status %s",
                e.response.status_code,
                exc_info=True,
            )
            raise RuntimeError(
                f"Direct embeddings request failed with status {e.response.status_code}"
            ) from e
        except Exception as e:
            logger.error(
                f"Failed to generate embeddings via direct endpoint: {e}", exc_info=True
            )
            raise RuntimeError(f"Failed to generate embeddings via direct endpoint: {e}") from e
    ...
    def _encode_raw_array(
        ...
        return self._encode_direct(texts, response_format="raw_array")
    ...
    def encode(
        ...
        """
        if self.response_format in {"raw_array", "openai_http"}:
            return self._encode_direct(texts, response_format=self.response_format)
