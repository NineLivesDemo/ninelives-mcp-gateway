...
> Historical design brief. The current deployment uses `terraform/env/dev/`
> and the implemented modules under `terraform/modules/`; this document's
> remaining phases describe work after the Azure foundation, not pending
> creation of the original `platform-pilot` layout.
...
The clean replacement is deployed in four lifecycle-scoped resource groups.
The existing Bicep environment and protected legacy resources remain outside
